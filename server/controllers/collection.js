...
var returnList1 = function(req, res, next) {
  var chain = db
  .select('name, secondary_title, masterid, out("HasStatus").name as status, type, in("FeaturedIn").size() as featured_count, in("FeaturedIn").name as featured_in, "collection" as entityType, type, both("Added").name as added, timestamp')
  .from('Collection');
