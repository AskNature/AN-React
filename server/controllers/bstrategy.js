...
var Strategy = require('../models/bstrategy.js');

if(process.env.NODE_ENV === 'production') {
    strategyCache = new Cached('bstrategy', { backend: {
	type: 'memcached',
	hosts: '127.0.0.1:11211'
    }});
} else {
    strategyCache = new Cached('bstrategy');
}
...
var loadindex = function(req, res, next) {
  // Render index.html to allow application to handle routing
   ...
   console.log('The bstrategy page has access to the ' + db.name + ' database.');
};

var returnList1 = function(req, res, next) {
  var chain = db
  .select('name, summary as description, out("HasLivingSystem").name as living_system, out("HasFunction").name as outcomes, out("HasMechanism").name as mechanisms, masterid, "b.strategy" as entityType, out("HasMedia")[0].filename as media, out("HasMedia")[0].entity as media_entity, out("HasMedia")[0].media_url as media_url, timestamp, both("AddedContent").name as addedby, out("HasStatus").name as status, flag_text, flag_tags, flag_media, is_deleted')
  .from('Strategy');
  ...
  strategyCache.getOrElse('count', Cached.deferred(function(done) {
	  ...
	  console.log('The bstrategy controller has sent ' + results.length + ' records.');
      }).done();
  });
};
...
var updateItem2 = function(req, res, next) {
	...
	if(!item) {
	    return res.status(404).send("No bstrategy with that id exists");
	} else {
...
var createBStrategy1 = function(req, res, next) {
	    ...
	    if(err) { return res.status(500).send(); }
            ...
            .then(function(count) {
                if(count > 0) {
                    ...
                    .set({masterid: masterid, name: 'New bstrategy', status: 'raw'}) // TODO: Proper template
...
var updateBStrategy1 = function(req, res, next) {
    ...
    db.update('Strategy').set(newData)
        .where({masterid:req.params.id}).scalar().then(function(count) {
            console.log("bstrategy updated: " + count);
...
var returnItem1 = function(req, res, next) {
  ...
  .done();
    ...
    module.exports = {
      ...
      updateBStrategy1: updateBStrategy1,
