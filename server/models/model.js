 ...
 * @class
 * @property {string} entityName The name of the OrientDB class this model is bound to.
 * @property {string[]} fields A list of the fields that belong in the schema.
...
var ConstructModel = function(entityName, fields, relationships) {
	...
	// assigns provided attributes that exist in the schema to their proper fields
	_.assign(this, _.pick(attributes, fields));

	// assigns the provided masterid
	this.masterid = masterid;

	// builds a nested model for each relationship from provided attributes
	_.forEach(relationships, function(val, key) {
		...
		// defines a place to store the resulting array of relationship data
		var arr;

		// if an array of data was provided in the attributes on construction,
		// then absorb that data
                if(_.isArray(attributes[key])) {
		    console.log(key + ' is array');
                    arr = attributes[key];

		// if not, then try to parse JSON data provided (necessary when client provides a result)
                } else {
                    try {
		...
		// if the value is of select type, then pull out its masterid from options
		if (val.select) {
